...
process_json() {
    ...
    jq -r '[.[] | 
       select(.transaction.messages[].["@type"] == "/cosmos.gov.v1beta1.MsgVote") |
       . += {
           "@type": (.transaction.messages[] | select(.["@type"] == "/cosmos.gov.v1beta1.MsgVote") | .["@type"]),
           "voter": (.transaction.messages[] | select(.["@type"] == "/cosmos.gov.v1beta1.MsgVote") | .voter)
       } |
