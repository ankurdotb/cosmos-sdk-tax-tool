...
class KoinlyConverter:
    ...
    def process_transaction(self, tx: Dict) -> Dict:
        ...
        if not tx_data:
            self.logger.debug("Empty transaction data found")
            return None
        ...
        has_non_client_updates = any(msg.get("@type") != "/ibc.core.client.v1.MsgUpdateClient" for msg in messages)
        ...
        for msg in messages:
            ...
            if msg_type == "/cosmos.bank.v1beta1.MsgSend":
                ...
                if is_sender:
                    ...
                    record["Description"] = (
                        f"Transfer of {amount} CHEQ from {msg['from_address']} to {msg['to_address']}"
                    )
            ...
            # Authz Grant
                ...
                auth_type = (
                    msg["grant"]["authorization"].get("@type", "").split(".")[-1]
